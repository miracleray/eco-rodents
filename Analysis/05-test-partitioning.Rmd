...
mean.pca <- plotTangentSpace(mean.shapes, axis1 = 1, axis2 = 2, verbose = T)
PlotPCA(mean.pca, 1, 2, alpha.col, alpha.pch, flip.axis1 = T)
```
...
### Then, plot residuals
```{r}
res.PCA <- plotTangentSpace(Allom.all.plus.consensus, groups = alpha.col)

PlotPCA(res.PCA, 1, 2, alpha.col, alpha.pch, flip.axis1 = TRUE, flip.axis2 = TRUE)
...
PlotPCA(res.PCA, 1, 2, alpha.col, alpha.pch)
legend("bottomright", phylo.names, col = col.unique.spp, pch = pch.unique.spp, pt.bg = col.unique.spp, cex = .53, pt.cex = .7, ncol = 2)
```
...
```{r}
save(alpha.col, alpha.pch, res.PCA, allom.resid, Allom.all.plus.consensus, file = "../Data/Processed/05-residual-data.rda")
```
...
```{r}
test.data <- info[, c("Taxa", "Genus", "Species", "CatNum", "Order", "Csize", "Feeding", "Guild")]  # relevant columns
