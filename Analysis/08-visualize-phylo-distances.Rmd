...
for (i in 1:length(region.names)) {
       ...
       ## For each species left in spp1, ID spp that never overlap and keep only those pairs (different biome pairs)
        for(i in 1:dim(spp1.region)[1]){
                # Make region.vector of regions each spp is in
                
                # Test for spp that A) exist in one or more of these regions AND B) don't exist in vector above; include passing spp in diff.vector
                
                # Keep only pairs with species[i] and spp in diff.vector
        }
        
        # Plot
        plot(diff.region.pairs$phylo.d, 
             diff.region.pairs$morph.d, 
             col = diff.region.pairs$col.spp1, 
             bg = diff.region.pairs$col.spp2, 
             pch = 21,
             main = paste(region.names[i], "phylo/morph distance plot"),
             xlab = "Phylogenetic distance (~million years since LCA)",  ylab = "Morphological distance")
}
