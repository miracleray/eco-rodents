...
carn <- c(which(alpha.names == "H chr"), which(alpha.names == "X myo"))

# No carnivores = nc
nc.info.means <- info.means[-carn, ]  # metadata
nc.residuals <- Allom.all.plus.consensus[, , -carn]  # shape data

## Decide here whether to use the hypothetical shape or not
nc.residuals <- allom.resid[, , -carn]  # Hypothetical shape
nc.residuals <- Allom.all.plus.consensus[, , -carn]  # Actual shape

# For plotting
nc.alpha.col <- alpha.col[-carn]  # colors
nc.alpha.pch <- alpha.pch[-carn]  # pch synbols
nc.alpha.names <- alpha.names[-carn]  # names (for plot labels)

# For legend (in phylogenetic order)
carn.p <- c(which(phylo.names == "H chr"), which(phylo.names == "X myo"))
nc.phylo.col <- col.unique.spp[-carn.p]  # colors
nc.phylo.pch <- pch.unique.spp[-carn.p]  # pch synbols
nc.phylo.names <- phylo.names[-carn.p]  # names
```

### Plot residuals
Color and legend by species
```{r}
nc.res.PCA <- plotTangentSpace(nc.residuals, groups = nc.alpha.col, label = nc.alpha.names)
...
PCA.plot <- plot(x = nc.res.PCA$pc.scores[, 1],
                 y = -nc.res.PCA$pc.scores[, 2],  # Flip for consistency
                 xlab = PCx.lab, 
                 ylab = PCy.lab,
                 asp = TRUE,
                 #xlim = c(-0.12, 0.2),
                 ...
                 cex.axis = 1.3, 
                 cex.lab = 1.3)
legend("bottomright", nc.phylo.names, col = nc.phylo.col, pch = nc.phylo.pch, pt.bg = nc.phylo.col, cex = .57, pt.cex = .7, ncol = 2)
```
...
```{r}
# No Notomys = not
not <- which(nc.info.means$Genus == "Not")
not.info.means <- nc.info.means[-not, ]  # metadata
...
# For legend
noto <- which(grepl("N ", names(nc.phylo.cols)))
