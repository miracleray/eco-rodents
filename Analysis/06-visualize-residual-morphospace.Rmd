...
```{r}
# Ordinate the data
twoD.shape <- two.d.array(residuals)
...
res.gpagen.shape$coords <- residuals
...
procrustes.var.plot(all.consensus, smmallest.shape) 
```

To test the hypothesis that each landmark's variation from the mean configuration is above expected, we use the smallest and largest specimens
```{r}
# Ordinate the data
twoD.shape <- two.d.array(residuals)
ordination <- stats::prcomp(twoD.shape)
...
The next step is to make a heatmap showing the size of variation and direction of movement for each landmark in the dataset, comparing between the median to both the min and max centroid size specimens.
```{r}
# Wrap specimens on the tangent space
res.wrap.PCA <- plotTangentSpace(residuals, verbose = FALSE)
