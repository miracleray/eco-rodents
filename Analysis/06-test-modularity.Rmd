...
alpha.key <- match(unique(lm.modules[, 4]), int.levels)  # alphabetize table
...
# 2) Plot of observed and resampled pls coefficients (w/ phylo correction)
...
title(xlab = "Integration: r-pls coefficient", line = 2.4)
...
# 3) Plot of pls coefficients (w/ phylo correction) for shape residual dataset
par(mar = c(3.5, 3.5, 1.5, 0.5))
...
#4 plot of observed and resampled CR coefficients (for full dataset/w phylocorrection)
...
title(xlab = "Modularity:CR coefficient", line = 2.4)
title(ylab = "Density", line = 2.35)
...
# 5) Plot of pls coefficients (w/ phylo correction) for shape residual dataset
par(mar = c(3.5, 3.5, 1.5, 0.5))
...
#thsi screams at you but just because of the NAs. the values are fine.
...
Mantel_matrix <- matrix(NA, nrow = length(modules), ncol = length(modules))
#         
...
diag(Mantel_matrix)<- n.num
 ...
 rownames(Mantel_matrix)<- modules
 colnames(Mantel_matrix)<- modules
