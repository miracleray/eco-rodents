...
### Old code to get specific species
```{r}
# Find min and max shapes    
min.name <- names(which(mean.PCA$pc.scores[, 1] == max(mean.PCA$pc.scores[, 1])))  # min because axis is flipped relative to our plots
max.name <- names(which(mean.PCA$pc.scores[, 1] == min(mean.PCA$pc.scores[, 1])))  # max
PC.allom.min <- mean.shapes[, , which(dimnames(mean.shapes)[[3]] == min.name)]
PC.allom.max <- mean.shapes[, , which(dimnames(mean.shapes)[[3]] == max.name)]
```
...
```{r}
# Find min and max shapes    
r.min.name <- names(which(res.PCA$pc.scores[, 1] == max(res.PCA$pc.scores[, 1])))
r.max.name <- "Mesembriomys_gouldii"  # because H. chr is extreme on PC2
PC.res.min <- residuals[, , which(dimnames(residuals)[[3]] == r.min.name)]
PC.res.max <- residuals[, , which(dimnames(residuals)[[3]] == r.max.name)]
...
# Select extreme specimens (the ones that make the warp-meshes)
res.PC2.min.h <- wrap.PCA.r$pc.shapes[[4]]  # flipped b/c axes are flipped
...
r2.max.name <- names(which(res.PCA$pc.scores[, 2] == min(res.PCA$pc.scores[, 2])))
r2.min.name <- names(which(res.PCA$pc.scores[, 2] == max(res.PCA$pc.scores[, 2])))
PC.res2.min <- residuals[, , which(dimnames(residuals)[[3]] == r2.min.name)]
PC.res2.max <- residuals[, , which(dimnames(residuals)[[3]] == r2.max.name)]
...
### Old code
Replace P. nov for N. cer and export plots of variation for residual shape change.
```{r}
# Find min and max shapes    
r2.min.n.name <- "Pseudomys_novaehollandiae"
r2.max.n.name <- "Mesembriomys_gouldii"
PC.res2.n.min <- residuals[, , which(dimnames(residuals)[[3]] == r2.min.n.name)]
PC.res2.n.max <- residuals[, , which(dimnames(residuals)[[3]] == r2.max.n.name)]
```
...
# Measure spherical coordinates differences from the mean for each specimen for each landmark
res.diff.mean.no <- coordinates.difference(coordinates = residuals.no, reference = res.mean, type = "spherical")
...
gpagen.shape.no$consensus <- res.mean
...
# Select extreme specimens (the ones that make the warp-meshes)
res.PC1.min.h.n <- wrap.PCA.r.n$pc.shapes[[2]]  # Flipped b/c axes are flipped
res.PC1.max.h.n <- wrap.PCA.r.n$pc.shapes[[1]]
```

### Visualize new PCA
```{r}
alpha.col.no <- alpha.col[-c(index.not, 38)]
alpha.pch.no <- alpha.pch[-c(index.not, 38)]
PlotPCA(wrap.PCA.r.n, 1, 2, alpha.col.no, alpha.pch.no)
legend("topright", legend = names(alpha.col.no), alpha.col.no, pch = alpha.pch.no, pt.bg = alpha.col.no, cex = 0.535, pt.cex = .7, ncol = 2)
```
