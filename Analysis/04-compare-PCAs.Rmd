...
# Get legend ready with native vs invasive Rattus
genera.phylo.full <- c("Mus", "Rattus (invasive)", "Rattus (native)", "Pogonomys", "Xeromys", "Hydromys", "Notomys", "Pseudomys", "Zyzomys", "Leggadina", "Uromys", "Melomys", "Leporillus", "Mesembriomys", "Conilurus")

col.phylo.full <- c(col.phylo[1], "black", col.phylo[2:length(col.phylo)])  # colors
```
...
par(mar = c(4.5, 4.5, 2, 1))
plot(x = res.PCA$pc.scores[, 1],  # no flip for consistency
...
plot(x = wrap.PCA.r.n$pc.scores[, 1],  
                 y = wrap.PCA.r.n$pc.scores[, 2],  # no flip for consistency
                 xlab = PCx.lab.n, 
                 ylab = PCy.lab.n,
                 asp = TRUE,
                 xlim = c(-0.11, 0.09),
