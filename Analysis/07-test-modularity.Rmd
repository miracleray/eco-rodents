...
R_pls_matrix[upper.tri(R_pls_matrix)] <- round(Combination_matrix$p, 3)
...
# integration_analyses$`basicran X molar`
# phylo.integration(rostrum, molar, phy=aus.tree)
# integration_analyses$`rostrum X molar`
# phylo.integration(molar, rostrum, phy=aus.tree)
# integration_analyses$`molar X rostrum`
...
R_pls_matrix_res[upper.tri(R_pls_matrix_res)] <- round(Combination_matrix$p, 3)
...
full_vs_res_int_strengths <- compare.pls(int.test.5.res, int.test.5.phy)
...
#and because it is a lot of coding to get to this point, and a lot of potential for human error, here are two manual checks. 
...
#Unlike phylo.integration, phylo.modularity provides a pairwise table. Here, upper is full data and lower is residuals
```{r}
