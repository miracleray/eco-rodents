...
colnames(mod.result) <- c("vault", "orbital", "basicran", "rostrum")
rownames(mod.result) <- c("orbital", "basicran", "rostrum", "molar")

# And now a test with the phylogentic tree (and mean shapes)
mod.test.phy <- phylo.modularity(mean.shapes, partition.gp = lm.modules[, 4], aus.tree)
```
...
```{r}
cme.rates <- compare.multi.evol.rates(mean.shapes, gp = as.character(lm.modules[, 3]), aus.tree)
