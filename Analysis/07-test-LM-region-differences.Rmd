...
for (i in 1:length(PartLevels)) {
    spheres3d(ref[Part[[i]], 1], ref[Part[[i]], 2], ref[Part[[i]], 3], col = Colors[i], lit = TRUE, radius = 0.001, asp = FALSE)
  rgl.texts(ref[Part[[i]], 1], ref[Part[[i]], 2], ref[Part[[i]], 3], lm.modules[, 4], offset = 100, color = c("black"))
}
...
mat <- matrix(c(1,2), 1, 2, byrow = TRUE) # 2 plots, 1 row, 2 columns
...
### Whole integration test - export plot for supplementary
