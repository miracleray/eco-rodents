...
This test uses covariance ratio (CR) coefficients (Adams 2016) to quantify the degree of modularity between hypothesized modules/partitions. The observed CR is compared to the distribution of randomized CR's, found by randomizing the landmarks into partitions of identical sample sizes.
```{r}
# Test with the phylogentic tree (and mean shapes)
mod.test.5 <- modularity.test(mean.shapes, partition.gp = lm.modules[, 4])

# Make pairwise comparison table
mod.result <- as.matrix(mod.test.5$CR.mat)  # results
colnames(mod.result) <- unique(lm.modules[, 4])  # names from 1st instance
rownames(mod.result) <- unique(lm.modules[, 4])  # ...in lm.module[, 4]
alpha.key <- match(unique(lm.modules[, 4]), mod.levels)  # alphabetize table
mod.result <- mod.result[order(alpha.key), order(alpha.key)]
```

## Test modularity with 5 modules for residual shape dataset
As above but with residuals
```{r}
# Test with the phylogentic tree (and mean shapes)
mod.test.res <- modularity.test(residuals, partition.gp = lm.modules[, 4])

# Make pairwise comparison table
mod.result.res1 <- as.matrix(mod.test.res$CR.mat)  # results
colnames(mod.result.res1) <- unique(lm.modules[, 4])  # names from 1st instance
rownames(mod.result.res1) <- unique(lm.modules[, 4])  # ...in lm.module[, 4]
mod.result.res1 <- mod.result.res1[order(alpha.key), order(alpha.key)]
```

## Test modularity with 5 modules for full shape dataset -- with phylogeny
As above but accounts for random changes due to evolution via Brownian motion (Felice and Adams 2014).
```{r}
# Test with the phylogentic tree (and mean shapes)
...
## Test modularity with 5 modules for residual shape dataset -- with phylogeny
As above but with residuals
```{r}
# Test with the phylogentic tree (and mean shapes)
...
save(mod.test.5, mod.test.res, mod.test.5.phy, mod.test.5.res, mod.result, mod.result.res1, mod.result.phy, mod.result.res, file = "../Data/Processed/07-modularity.rda")
```

## Make and export Figure 4
```{r}
# Set up for export and for multiple panels
pdf("../Data/Results/Figure4_Modularity_new.pdf")
mat <- matrix(c(1, 1, 2, 3, 4, 5), 3, 2, byrow = TRUE)  # 5 plots, 3 row, 2 columns
...
legend("topleft", "module definitions", cex = 0.9, bty = "n", inset = c(0.03, 0.03))
legend("topleft", "(observed condition)", cex = 0.9, bty = "n", inset = c(0.03, 0.1))
text(80, 0, "Basicranium", cex = 0.9, bty = "n", col = darken(mod.colors[1]))
...
text(0, 190, "Vault", cex = 0.9, bty = "n", col = darken(mod.colors[5], amount = 0.35))

# 2) Plot of observed and resampled CR coefficients for full shape dataset
par(mar = c(3.5, 3.5, 1.5, 0.5))
hist(mod.test.5$random.CR,
     main = NULL,
     xlab = "",
     xlim = c(0.65, 1.1),
     ylab = "",
...
arrows(x0 = mod.test.5$CR, y0 = 465, y1 = 8, length = 0.15, lwd = 1.5, col = "darkred")
text(mod.test.5$CR - 0.001, 500, "observed CR", cex = 0.9, col = "darkred")
legend("topleft", "B", cex = 1.5, bty = "n", inset = -0.05)
legend("topleft", "resampled CR coefficients; full shape", cex = 0.9, bty = "n", inset = c(0.1, 0.02))

# 3) Plot of CR coefficients for residual shape dataset
par(mar = c(3.5, 3.5, 1.5, 0.5))
hist(mod.test.res$random.CR,
     main = NULL,
     xlab = "",
     xlim = c(0.65, 1.1),
     ylab = "",
...
title(xlab = "Covariance ratio (CR) coefficient", line = 2.4)
title(ylab = "Frequency (out of 1000)", line = 2.35)
arrows(x0 = mod.test.res$CR, y0 = 375, y1 = 8, length = 0.15, lwd = 1.5, col = "darkred")
text(mod.test.res$CR + 0.02, 400, "observed CR", cex = 0.9, col = "darkred")
legend("topleft", "C", cex = 1.5, bty = "n", inset = -0.05)
legend("topleft", "resampled CR coefficients; residual shape", cex = 0.9, bty = "n", inset = 0.02)

# 4) Plot of observed and resampled CR coefficients (w/ phylo correction)
par(mar = c(3.5, 3.5, 1.5, 0.5))
...
legend("topleft", "as above, with phylogenetic correction", cex = 0.9, bty = "n", inset = 0.02)

# 5) Plot of CR coefficients (w/ phylo correction) for residual shape dataset
par(mar = c(3.5, 3.5, 1.5, 0.5))
...
text(mod.test.5.res$CR - 0.001, 125, "observed CR", cex = 0.9, col = "darkred")
...
# Run on the full and residual shape datasets respectively
