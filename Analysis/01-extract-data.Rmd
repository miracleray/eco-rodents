...
#run bilateraly symmetry analyses of all modules with the module pairs. First just checking how the "ind" vector works in Ariel's code - it's the same as the dimnames. So I am using just the dimnames as the "ind" vector for the modules
...
module_bilat [[i]] <- bilat.symmetry(module_gpas[[i]]$coords, ind = dimnames(module_gpas[[i]]$coords)[[3]], object.sym = TRUE, replicate = NULL, side = NULL, land.pairs = module_pairs[[i]])
...
module_bilat [[i]]$Csize <- module_csizes[[i]]
...
```{r}
save(big.bY.shape, big.sp.info, sm.bY.shape, sm.sp.info, rep.spec, big.pt.names, sm.pt.names, module_bilat, file = "../Data/Processed/01-bilat-data-w-ERR.rda")
