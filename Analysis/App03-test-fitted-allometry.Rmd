---
title: "App03-test-fitted-allometry"
author: "Ariel Marcy"
date: "3/5/2020"
...
This script produces Supplementary Figure TK and is relevant to Figure 2's second column, which is currently using the method from #1 (script 06).  
...
Exact same code as script 06; can skip if script 06 has been run.
```{r}
# Fit specimens
...
```{r}
# Set up muli-panel figure TK; lateral 
setEPS() 
postscript("../Data/Results/SFigure_Allometry_Compare.eps")
...
# 5) Mean to Small
par(mar = c(0, 0, 0, 0))  # sets the margins
plot(c(0, dim(GMfit.sm.lat)[2]), c(0, dim(GMfit.sm.lat)[1]), type = "n", axes = F, xlab = "", ylab = "", asp = T) 
...
# 6) Mean to Big
par(mar = c(0, 0, 0, 0))
plot(c(0, dim(GMfit.big.lat)[2]), c(0, dim(GMfit.big.lat)[1]), type = "n", axes = F, xlab = "", ylab = "", asp = T) 
...
# 7) Mean to Small
par(mar = c(0, 0, 0, 0))  # sets the margins
plot(c(0, dim(GMfit.big.dor)[2]), c(0, dim(GMfit.big.dor)[1]), type = "n", axes = F, xlab = "", ylab = "", asp = T) 
...
# 9) Mean to Small
par(mar = c(0, 0, 0, 0))
plot(c(0, dim(regsc.sm.lat)[2]), c(0, dim(regsc.sm.lat)[1]), type = "n", axes = F, xlab = "", ylab = "", asp = T) 
...
# 10) Mean to Big
par(mar = c(0, 0, 0, 0))
plot(c(0, dim(regsc.big.lat)[2]), c(0, dim(regsc.big.lat)[1]), type = "n", axes = F, xlab = "", ylab = "", asp = T) 
...
# 11) Mean to Small
par(mar = c(0, 0, 0, 0))
plot(c(0, dim(regsc.big.dor)[2]), c(0, dim(regsc.big.dor)[1]), type = "n", axes = F, xlab = "", ylab = "", asp = T) 
