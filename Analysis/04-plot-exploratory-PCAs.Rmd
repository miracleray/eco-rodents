...
load(file = "../Data/Processed/allometry_rodents_04-phylo-data.rda")  # brings in phylogenetic information to organize legends
```
...
Finally, we'll combine these functions in a loop to plot PCAs for each region, with unique species within each genera having a unique point shape. The legends only name the species present for a given region. 
```{r}
...
for (i in 1:7) {
        ...
        PCA.plot <- plot(x = pc.region$PC1,
                         ...
                         cex.axis = 1.3, 
                         cex.lab = 1.3)
        legend("bottomright", legend = unique.taxa, col = col.gen.legend, border = NULL, pch = pch.tax.legend, pt.bg = col.gen.legend, cex = 0.64)
}
```

### Save intermediate data
```{r}
save(info.traits, col.diet, pch.gld, file = "../Data/Processed/04-sm-data-with-traits.rda")
```