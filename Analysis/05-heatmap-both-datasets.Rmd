...
res.PC1.min.h <- wrap.PCA.r$pc.shapes$PC1min
res.PC1.max.h <- wrap.PCA.r$pc.shapes$PC1max
```
...
# Select extreme specimens (the ones that make the warp-meshes)
res.PC2.min.h <- wrap.PCA.r$pc.shapes$PC2max  # switch for consistency
res.PC2.max.h <- wrap.PCA.r$pc.shapes$PC2min
...
# Select extreme specimens (the ones that make the warp-meshes)
res.PC1.min.h.n <- wrap.PCA.r.n$pc.shapes$PC1min 
res.PC1.max.h.n <- wrap.PCA.r.n$pc.shapes$PC1max
```
...
```{r}
# Select extreme specimens (the ones that make the warp-meshes)
res.PC2.min.h.n2 <- wrap.PCA.r.n$pc.shapes$PC2min
res.PC2.max.h.n2 <- wrap.PCA.r.n$pc.shapes$PC2max
...
## 3) Make multi-panel Figure 3: Heatmaps with Residuals - NEW VERSION
```{r}
# Set up muli-panel figure 
pdf("../Data/Results/Figure3_Heatmap_Residuals_new.pdf")
mat <- matrix(c(1:12), 4, 3, byrow = FALSE)  # 12 plots, 4 rows, 3 columns
layout(mat, widths = rep(1, dim(mat)[2]), heights = rep(0.5, dim(mat)[1])) 
...
## 3) Make multi-panel Figure 3: Heatmaps with Residuals - OLD VERSION
```{r}
# Set up muli-panel figure 
