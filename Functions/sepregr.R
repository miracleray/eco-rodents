sepregr<-function(x,y,group,scalevariable=1,conf=0,nperm=999,labels=NULL,reorder=T){
  library(MASS)
  library(car)
  library(vegan)
  library(CCA)
  ...
  out<-(list(x=x,y=y,group=group,pooled_correlation=pooled_correlation,regression_results=mat.test,PCsony=PCsony,predicted=mat.predfin,
             residuals=mat.residfin,obsatmin=obsatmin, obsatmax=obsatmax,residplusmax=residplusmaxfin,residplusmin=residplusminfin,coefficients=mat.coef,PCsonpred=PCsonpred,varpcsonpred=varpcsonpred,ccascores=ccascores,predccascores=mat.predccascores,
             
             plot=if (ncol(PCsonpred)>=3){ plot3D(PCsonpred[,1:3],col=as.numeric(group),bbox=F,type="s",cex=scalevariable)} else {NULL}, 
             axis1=if (ncol(PCsonpred)>=3){arrows3D(c(max(PCsonpred[,1:3]),0,0),c(-max(PCsonpred[,1:3]),0,0),length=0.03,lwd=100,add=T,labs="1PConpred")} else {NULL} ,
             axis2=if (ncol(PCsonpred)>=3){arrows3D(c(0,max(PCsonpred[,1:3]),0),c(0,-max(PCsonpred[,1:3]),0),length=0.03,add=T,labs="2PConpred")} else {NULL},
             axis3=if (ncol(PCsonpred)>=3){arrows3D(c(0,0,max(PCsonpred[,1:3])),c(0,0,-max(PCsonpred[,1:3])),length=0.03,add=T,labs="3PConpred")} else {NULL},
             x11=x11(),
             plot2=plot(x,ccascores[,2],cex=scalevariable,col=as.numeric(group),ylim=range(ccascores[,2])),
             par=par(new=T)  ,
             plot3=plot(x,mat.predccascores,cex=scalevariable,col=as.numeric(group),pch=20,ylim=range(ccascores[,2])),  
             #newscene=open3d(),
             plot6=if (ncol(PCsonpred)>=2){plot3dcol(cbind(x,PCsonpred[,1:2]),group,scalevariable,reorder=reorder,legend=F,axlab=c("x","1PConpred","2PConpred"))} else {NULL},  
             ellipses=for(i in 1:max(species)){
               par=par(new=T) 
               dataEllipse(x[species==i],ccascores[,2][species==i],col=i,add=T,levels=c(conf), center.pch=18, center.cex=scalevariable*2,plot.points=F,fill=T)
             },
             x11=x11(),
             plot5=plot(x,lm(mat.predccascores~x*group)$fitted.values,col=as.numeric(group),pch=20,cex=scalevariable), 
             tt=if(!is.null(labels)){textxy(x,lm(mat.predccascores~x*group)$fitted.values,labels)}else{NULL}
